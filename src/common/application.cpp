...
GLFWwindow *
    ...
    ImFont* font = io.Fonts->AddFontFromFileTTF(Data::resolve("Ubuntu.ttf").string().c_str(), 26);
...
void Application::window_character_callback(GLFWwindow * window, unsigned int codepoint) {
    auto * app = static_cast<Application *>(glfwGetWindowUserPointer(window));
    app->character_callback(codepoint);
}

void Application::window_cursor_position_callback(GLFWwindow * window, double xpos, double ypos) {
    auto * app = static_cast<Application *>(glfwGetWindowUserPointer(window));
...
void Application::window_scroll_callback(GLFWwindow * window, double xoffset, double yoffset) {
    auto * app = static_cast<Application *>(glfwGetWindowUserPointer(window));
    app->scroll_callback(xoffset, yoffset);
}

void Application::window_mouse_button_callback(GLFWwindow * window, int button, int action, int mods) {
    auto * app = static_cast<Application *>(glfwGetWindowUserPointer(window));
    app->mouse_button_callback(button, action, mods);
}

void Application::window_cursor_enter_callback(GLFWwindow * window, int entered) {
    auto * app = static_cast<Application *>(glfwGetWindowUserPointer(window));
...
void Application::draw_profiler_ui() const {
    ...
    auto & io = ImGui::GetIO();
    MicroProfileMousePosition(static_cast<int>(io.MousePos.x), static_cast<int>(io.MousePos.y), static_cast<int>(io.MouseWheel));

    if (! io.WantCaptureMouse) {
        MicroProfileMouseButton(io.MouseDown[ImGuiMouseButton_Left], io.MouseDown[ImGuiMouseButton_Right]);
    } else {
...
glm::vec3 ModelViewerCamera::position() const {
    return _distance * polar_to_cartesian(_yaw, _pitch) + glm::vec3(0.0f, _focus_height, 0.0f);
}
